...
from app.services.mock_signals import DEFAULT_SIGNALS
...
async def list_zones(db: AsyncSession = Depends(get_db)) -> list[ZoneScoreOut]:
    ...
    of London). weather_severity (Open-Meteo), traffic_congestion (TfL road
    status), transit_delay (TfL line status, all rail modes), and
    event_density (Ticketmaster Discovery API, when TICKETMASTER_API_KEY is
    configured) are all live and fall back to a mocked value per-signal if
    their fetch fails or isn't configured.
    """
    stmt = select(
        Zone,
    ...
    live_weather, live_traffic, live_transit, live_events = await asyncio.gather(
        asyncio.gather(
            *(fetch_weather_severity(lat, lng) for _, _, lat, lng, *_ in rows)
        ),
        asyncio.gather(
            *(
        ...
        asyncio.gather(
            *(fetch_event_density(lat, lng) for _, _, lat, lng, *_ in rows)
        ),
    )
    ...
    for (
        ...
        signals = DEFAULT_SIGNALS.model_copy(
            update={
                ...
                else DEFAULT_SIGNALS.weather_severity,
                "traffic_congestion": traffic_congestion
                if traffic_congestion is not None
                else DEFAULT_SIGNALS.traffic_congestion,
                "transit_delay": transit_delay
                if transit_delay is not None
                else DEFAULT_SIGNALS.transit_delay,
                "event_density": event_density
                if event_density is not None
                else DEFAULT_SIGNALS.event_density,
            }
        )
        zones.append(
            ZoneScoreOut(
